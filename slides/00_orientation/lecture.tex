...
% --- [Preamble] TikZ 스타일 전역 정의 (Starlink) ---
\tikzset{
...
% --- [Preamble] 피라미드 다이어그램 스타일 정의 ---
\tikzset{
    % 기본 레이어 스타일 (둥근 모서리 사각형, 흰색 굵은 글씨)
    pyramid_layer/.style={
        rectangle, rounded corners=5pt, draw=white, line width=2pt,
        align=center, font=\sffamily\bfseries\small, text=white,
        drop shadow={opacity=0.3}
    },
    % 최하단 기반층 (붉은색 강조)
    foundation_layer/.style={
        pyramid_layer, fill=red!80!black, minimum height=1.8cm, minimum width=10cm
    },
    % 중간 기술층 (파란색)
    middle_layer/.style={
        pyramid_layer, fill=blue!60!cyan, minimum height=1.5cm, minimum width=7.5cm
    },
    % 최상단 응용층 (초록색/청록색)
    top_layer/.style={
        pyramid_layer, fill=teal!70!green, minimum height=1.5cm, minimum width=5cm
    }
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\frametitle{우리나라의 주요 주파수 대역별 용도}
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth, height=0.85\textheight, keepaspectratio]{freq_distribution}
...
% Summary
\section*{Summary}
\begin{frame}{The Role of Communication Theory}
\centering

    % 상단 문구 (Option 1을 기본으로 설정, 필요시 주석 해제하여 변경)
    \Large \bfseries
    % [Option 1] 시스템 관점
    ``Communication Theory is \textcolor{red}{the Foundation} of Modern Digital \& Wireless Systems.''
    % [Option 2] 본질 관점
    % ``Communication Theory is \textcolor{red}{the Foundation} of Reliable Information Exchange.''

    \vspace{0.5cm}

    \resizebox{0.9\textwidth}{!}{%
    \begin{tikzpicture}[node distance=0.3cm]
        % --- 1. Foundation Layer (최하단) ---
        \node[foundation_layer] (bottom) {
            \Large COMMUNICATION THEORY\\[0.3em]
            \normalsize (Signals, Noise, Probability, Limits)
        };

        % --- 2. Middle Layer (중간 기술) ---
        \node[middle_layer, above=of bottom] (middle) {
            \normalsize Essential Enabling Technologies\\[0.2em]
            \footnotesize (Modulation, Coding, DSP, Multiplexing)
        };

        % --- 3. Top Layer (최상단 응용) ---
        % [!] 교수님, 이곳에서 Option 1 또는 Option 2 텍스트를 선택하세요.
        \node[top_layer, above=of middle] (top) {
            % [Option 1 텍스트]
            \normalsize MODERN DIGITAL \& WIRELESS SYSTEMS\\[0.2em]
            \footnotesize (5G/6G, Satellite, WiFi, IoT)

            % [Option 2 텍스트] - 사용 시 위 3줄 주석처리하고 아래 주석 해제
            % \normalsize RELIABLE INFORMATION EXCHANGE\\[0.2em]
            % \footnotesize (Global Connectivity, Data Integrity)
        };

        % --- 4. 부가 요소 (화살표 및 라벨) ---
        % 상승 화살표 (Supporting Flow)
        \draw[->, ultra thick, gray!50, line width=5pt, opacity=0.7]
            ($(bottom.east)+(0.5, -0.5)$) to[out=45, in=-45] node[midway, right, font=\bfseries\small, text=gray] {Supports \& Enables} ($(top.east)+(0.5, 0.5)$);

        % 왼쪽 라벨 (The Foundation)
        \node[left=0.2cm of bottom, font=\bfseries\small, text=red!80!black, align=right] {The\\Foundation};
        
        % 왼쪽 라벨 (Real-world Impact)
        \node[left=0.2cm of top, font=\bfseries\small, text=teal!80!black, align=right] {Real-world\\Impact};
