...
\tcbuselibrary{skins, breakable}   % 박스 스킨 라이브러리
\usepackage{multicol}
...
\setbeamerfont{section}{size=\footnotesize,shape=\bfseries}
...
\begin{frame}[c]{Example 2.1}
    ...
    \begin{column}{0.6\textwidth}
        {\footnotesize
        Determine the suitable measures of the signals in the left Figure.
        {\scriptsize
        \begin{tcolorbox}[fonttitle=\bfseries,title=(a) Energy signal,title filled]
        ...
        \begin{tcolorbox}[fonttitle=\bfseries,title=(b) Power signal,title filled]
            Averaging $g^2(t)$ over an infinitely large interval is equivalent to averaging it over one period (2 seconds).
            \begin{eqnarray*}
...
\begin{columns}[c]
    % [왼쪽 컬럼] 그림 (a), (b)
    \begin{column}{0.35\textwidth}
    \begin{figure}[h, font=\footnotesize]
        \begin{tikzpicture}[scale=0.5, font=\scriptsize, >=latex]
            ...
            \end{scope}
        \end{tikzpicture}
    \caption{Continuous vs Discrete Time Signals}
    \end{figure}
    \end{column}
    ...
    \begin{column}{0.55\textwidth}
        ...
        \begin{tcolorbox}[enhanced,title=(a) Continuous Time Signal, frame style={left color=red!75!black, right color=blue!75!black}]
            Continous time signals are specified for every value of time $t$. Many examples including:
        \begin{itemize}
        ...
        \tcbset{colframe=red!5!black,fonttitle=\bfseries\footnotesize,fontupper=\scriptsize,fontlower=\scriptsize}
        \begin{tcolorbox}[enhanced,title=(b) Discrete Time Signal, frame style={left color=red!75!black, right color=blue!75!black}]
            Discrete time signals are specified only at discrete points of $t = nT$. Many examples including:
        \begin{itemize}
...
\begin{figure}[h]
    \begin{tikzpicture}[scale=0.7, font=\scriptsize, >=latex]
        ...
        \end{scope}
    \end{tikzpicture}
    \caption{Classification of signals based on amplitude and time domains}
\end{figure}
\end{frame}
...
\begin{frame}{Periodic and Aperiodic Signals}
    \small        
    \incfig{fig_ch02_05}{0.6}{Periodic signal of period $T_0$}

    A signal $g(t)$ is \textbf{periodic} if there exists a positive constant $T_0$.
    \addtocounter{equation}{4}
    \begin{equation}
    g(t) = g(t + T_0) \text{\quad for all t}
    \end{equation}
    The smallest value of $T_0$ in Eq. (\theequation) is the \textbf{period} of $g(t)$.
