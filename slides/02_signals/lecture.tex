...
\begin{frame}[fragile]
    ...
    \begin{columns}
        ...
        \begin{column}{0.55\textwidth}
            ...
            \begin{tcolorbox}[fonttitle=\sffamily\bfseries\small,title=\cref{fig:2.1}(a) Energy Signal]
            A signal is said to be an energy signal if its energy is finite and its average power approaches zero.
                \begin{equation}
            ...
            \begin{tcolorbox}[fonttitle=\sffamily\bfseries\small,title=\cref{fig:2.1}(b) Power Signal]
            A signal is said to be a power signal if its average power is finite and its energy approaches infinite.
                \begin{equation}
...
% ------------------------------------------------------------------------------
\begin{frame}{1. Low Frequency Bands (VLF $\sim$ MF)}
...
% ------------------------------------------------------------------------------
\begin{frame}{2. Mid-Range Frequency (HF $\sim$ VHF)}
    % [상단] 그림
    \begin{figure}
        \centering
...
% ------------------------------------------------------------------------------
\begin{frame}{3. High Frequency Bands (UHF $\sim$ EHF)}
    % [상단] 그림
    \begin{figure}
        \centering
...
\begin{frame}{Starlink Network Architecture}
    ...
    \resizebox{0.95\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node[text=signal_orange, font=\scshape\tiny, below=0.1cm of dish] {User Terminal};
            ...
            \node[text=signal_green, font=\tiny, rotate=-72] at (-4.1, 2.5) {1.9 GHz};

            % (3) User Dish Beam
            \path[beam_cone=signal_orange] 
                ($(S1)+(0.2,-0.2)$) -- ($(dish.north)+(-0.4,0)$) -- ($(dish.north)+(0.4,0)$) -- cycle;
            \node[text=signal_orange, font=\tiny, rotate=-30] at (-1.8, 2.5) {Ku-band};

            % (4) Gateway Beam
            \path[beam_cone=starlink_cyan] 
                ($(S2)+(0,-0.2)$) -- ($(gateway.north)+(-0.4,0)$) -- ($(gateway.north)+(0.4,0)$) -- cycle;
            \node[text=starlink_cyan, font=\tiny, rotate=-78] at (4.1, 2.5) {Ka/E-band};
        \end{tikzpicture}
    }
\end{frame}
